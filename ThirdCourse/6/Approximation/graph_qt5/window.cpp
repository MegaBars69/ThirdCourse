...
{  
  ...
  ChebyshovAproximation(n, F, alpha, g, z);
  */
  ...
  painter.setPen(QPen(Qt::lightGray, 0, Qt::DotLine)); // Устанавливаем серый цвет и пунктирный стиль
  
  double gridSpacing = 50; // Расстояние между линиями сетки
  /*
  // Рисуем вертикальные линии
  for (double x = a; x <= b; x += (b - a) / n) {
      painter.drawLine(L2G(x, min_y), L2G(x, max_y));
  }*/

  // Рисуем горизонтальные линии
  for (double y = min_y; y <= max_y; y += gridSpacing) {
      painter.drawLine(L2G(a, y), L2G(b, y));
  }
  // draw axis
